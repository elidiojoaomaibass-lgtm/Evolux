 ...
 */
export async function getLowtrackToken() {
    if (cachedToken)
        return cachedToken;
    const token = process.env.VITE_LOWTRAK_API_KEY ||
        process.env.VITE_MERCHANT_LOWTRACK_TOKEN ||
        process.env.LOWTRAK_API_KEY ||
        process.env.MERCHANT_LOWTRACK_TOKEN;
    if (!token) {
        throw new Error('LowTrack token not found. Define VITE_LOWTRAK_API_KEY (or related env vars) in .env.');
    }
    cachedToken = token;
    return token;
}
